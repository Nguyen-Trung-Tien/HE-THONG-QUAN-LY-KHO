...
const Card = ({ children, title, extra, className = '', noPadding = false, variant = 'default' }) => {
  const variants = {
    ...
    glass: 'bg-white/70 dark:bg-dark-card/60 backdrop-blur-xl border-white/40 dark:border-dark-border/30 shadow-soft-2xl',
  ...
  return (
    <div className={cn(
      ...
      {(title || extra) && (
        <div className="px-6 py-4 sm:px-8 sm:py-5 border-b border-border/40 dark:border-dark-border/40 flex items-center justify-between bg-gradient-to-r from-bg-subtle/20 dark:from-white/[0.01] to-transparent">
          {title && (
            <div className="flex items-center space-x-2">
              <div className="w-1 h-3 bg-primary rounded-full" />
              <h3 className="text-xs sm:text-sm font-black text-text-primary tracking-tighter uppercase">{title}</h3>
            </div>
          )}
