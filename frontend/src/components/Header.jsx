...
import { FiPlus, FiLogOut, FiSettings, FiUser, FiBell } from "react-icons/fi";
...
function Header({ onOpenSidebar }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (notiRef.current && !notiRef.ref?.contains(event.target)) {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-dark-card/80 backdrop-blur-md border-b border-border/50 dark:border-white/5 h-20 shadow-sm transition-all duration-500 px-4 sm:px-10">
        <div className="h-full flex items-center justify-between">
          <div className="flex items-center gap-x-4">
            ...
            <div className="hidden lg:block">
              <h2 className="text-[10px] font-semibold text-text-tertiary dark:text-dark-text-tertiary uppercase tracking-[0.3em] leading-none mb-1">
                System Platform
              </h2>
              <p className="text-xs font-black text-text-primary dark:text-dark-text-primary uppercase tracking-tighter">
                Control Panel <span className="text-primary">v3.0</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-x-4">
            ...
            <div className="relative" ref={notiRef}>
              <button
                ...
                className={cn(
                  ...
                  notiOpen ? "bg-primary text-white shadow-lg shadow-primary/30" : "bg-bg-subtle dark:bg-white/5 text-text-secondary dark:text-dark-text-secondary hover:bg-primary/5 hover:text-primary"
                )}
              >
            ...
            <div className="w-px h-8 bg-border/40 dark:bg-white/5 hidden sm:block" />

            <div className="flex items-center gap-x-3 sm:gap-x-5">
               <div className="flex items-center gap-x-1.5 sm:gap-x-3 group cursor-pointer" onClick={() => navigate('/profile')}>
                  <div className="flex flex-col items-end hidden sm:flex">
                     <span className="text-[11px] font-black text-text-primary dark:text-dark-text-primary uppercase tracking-tight leading-none mb-1 group-hover:text-primary transition-colors">
                  ...
                  <div className="size-10 rounded-2xl bg-bg-subtle dark:bg-white/5 border border-border/50 dark:border-white/10 p-0.5 group-hover:border-primary transition-all shadow-inner-sm overflow-hidden">
                     {currentUser?.image ? (
               ...
               <button 
                ...
                className="p-2.5 rounded-xl bg-bg-subtle dark:bg-white/5 text-text-tertiary hover:text-error hover:bg-error/5 transition-all duration-300"
                title={t('logout')}
               >
                 <FiLogOut size={20} />
               </button>
